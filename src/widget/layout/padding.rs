...
impl Widget for Padding {
    ...
    fn handle_user_event(&mut self, event: &UserEvent, widget_events: &mut Vec<WidgetEvent>) {
        // There is a child widget.
        if let Some(child_widget) = &mut self.child_widget {
            child_widget.handle_user_event(event, widget_events);
        }
    }
    ...
    fn handle_widget_command(&mut self, widget_command: &WidgetCommand) {
        match widget_command {
            WidgetCommand::Remove(widget_id) => {
                ...
                // There is a child widget.
                if let Some(child_widget) = &mut self.child_widget {
                    // Let the child widget handle the widget command.
                    child_widget.handle_widget_command(widget_command);
                }
            }
            WidgetCommand::SetHasFocus(widget_id, _has_focus) => {
                // A padding can not be focused.
                if *widget_id == self.widget_id {
                    return;
                }
            ...
            WidgetCommand::SetValue(widget_id, _value) => {
                // A value can not be set to a padding.
                if *widget_id == self.widget_id {
                    return;
                }
